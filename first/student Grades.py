...
class  StudentGradesInfoTester:
    ...
    def test_total_courses_cnt(cls):
        student = StudentGradesInfo('ID1234')
        if len(student.grades)==len(student.courses_names)==0:
            return True
        student.add_grade(70, "Math")
        if len(student.grades)==len(student.courses_names)==1:
            return True
        student.add_grade(70, "programming 1")
        if len(student.grades)==len(student.courses_names)==2:
            return True
        student.add_grade(85, "programming 2")
        if len(student.grades)==len(student.courses_names)==3:
            return True
...
if __name__ == '__main__':
    print(StudentGradesInfoTester.test_total_courses_cnt())
